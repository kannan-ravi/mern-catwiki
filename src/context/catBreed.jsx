...
export const CatBreedProvider = ({ children }) => {
  ...
  useEffect(() => {
    const popularSort = () => {
      ...
      const compareDescending = (a, b) => { return b.popular - a.popular }

      //! Sorting in Ascending Order for Breed Name
      const compareAscending = (a, b) => a.name < b.name ? -1 : a.name > b.name ? 1 : 0;

      //! Filter by Breed Name for Search result
      const filterBreedName = [...breedData].sort(compareAscending).filter(data => (data.name.toLowerCase()).includes(search.toLowerCase()));
  ...
  return (
    <CatBreedContext.Provider value={{
      breedData, imageData, setImageData, popularBreed, sortedName,
      search, setSearch, filterDisplay, singleBreedPage,
